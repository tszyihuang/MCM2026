"""问题3 的进程内精确复刻环境 (机器狗可见信息 与 真值 严格分离)。

物理规则与 simulator.core 逐位一致 (已由 solver/tests/test_equiv.py 逐动作验证):
移动 5 m/s, 检测 5 s, 切换频道 1 s, 清除命中 5 s / 未发现 3 s,
...
def _draw_case(seed: int) -> List[Source]:
    """核心生成循环, 与 ``CaseGenerator.generate(problem=3)`` 逐位一致。
...
def generate_case(seed: int) -> List[Source]:
    """全向源案例; 与 ``CaseGenerator.generate(problem=3)`` 逐位一致。"""
    return _draw_case(seed)
...
class World:
    """一局问题3。对外只暴露机器狗可见信息 (world.belief), 真值仅供评测。"""

    def __init__(self, seed: Optional[int] = None, run_id: str = "RL",
                 deterministic_error: bool = True,
                 belief_cls: Optional[type] = None) -> None:
        if seed is None:
            seed = random.randrange(1 << 30)
        self.seed = int(seed)
        self.run_id = run_id
        self.sources: List[Source] = generate_case(self.seed)
