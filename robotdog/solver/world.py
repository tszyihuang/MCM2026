...
注意 `/clear` 的成功判据**与覆盖角无关** (附件2 第8条只说距离) —— 覆盖角只影响
`/measure`。
"""
...
class World:
    ...
    def __init__(self, seed: Optional[int] = None, run_id: str = "RL",
        ...
        if belief_cls is None:
            if self.problem == 4:
                # 问题4 的 ψ 分箱信念 (定向方向推断)。问题4 规划器本身不使用信念 ——
                # 它通过 ``world_kwargs()`` 装一个空实现 —— 这里只是环境的默认值。
                from .belief4 import Belief4
                belief_cls = Belief4
            else:
                belief_cls = Belief
        self.belief = belief_cls()
