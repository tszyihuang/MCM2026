...
class SweepConfig:
    ...
    hunt_mass_min: float = 0.20       # 残余期望源数低于该值即收手 (验收判据 = 清除比例 >= 0.98)
                                      # 残余期望源数落在 [0.02, 0.20) 时每次专程探测的命中率
                                      # 只有 0.9%~3.2%, 代价却是 166~203 s 的往返
...
def build_cfg() -> "SweepConfig":
    ...
    最小化平均定位清除时间"标定。300 局标定集 (seed 9500-9799) 的关键取值与理由:
      ...
      * **收尾闸门 ``hunt_mass_min=0.20``** (唯一重要的旋钮): 残余期望源数低于该值
        即收手。残余期望源数落在 [0.02, 0.20) 时每次专程探测的命中率只有
        0.9%~3.2%, 代价却是 166~203 s 的往返, 因此按 0.98 判据把它放到 0.20;
...
def pursue_fast(world: World, c: int, cfg: SweepConfig,
    ...
    * ``bow=True`` (默认): 在**剩余路段的中点**、按
    ...
    实测 (300 局 9500-9799, 进程内): 平均定位清除 258.1 -> 250.5 s/源, 清除率
